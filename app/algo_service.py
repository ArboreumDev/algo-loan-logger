...
from utils.utils import (
    wait_for_confirmation, print_asset_holding, print_created_asset, get_created_asset, get_arc3_nft_metadata
)
from utils.types import InvalidAssetIDException, UnlockedAccount, NewLoanParams, NewLogAssetInput, LogData
import base64
import json
...
class AlgoService():
    def __init__(self, 
        algod_address: str, algod_token: str, indexer_token: str, indexer_address: str,
        master_mnemonic: str
    ):
        ...
        self.master_account = UnlockedAccount(
            public_key=mnemonic.to_public_key(master_mnemonic),
    ...
    def create_new_asset(self, input: NewLogAssetInput ):
        ...
        # sample_data = """{ "tenor_in_days": 90, "loan_id": "ll42", "principal": 200000, "apr": 0.13, "start_date": 1600942397, "invoices": ["35ce990e-d39c-4cb6-8335-eea9fc88d3fc", "75906861-abee-46c9-9a1b-56e65ddfa6f4"] }"""
...
def get_algo_client(net=".env"):
    ...
    if connect_to == "LOCAL" or connect_to == "SANDBOX":
        if connect_to == "LOCAL":
            # master_mnemonic = "enforce drive foster uniform cradle tired win arrow wasp melt cattle chronic sport dinosaur announce shell correct shed amused dismiss mother jazz task above hospital"
            master_mnemonic = os.getenv("LOCAL_MASTER_MNEMONIC")
        else: 
            # addr = "WWYNX3TKQYVEREVSW6QQP3SXSFOCE3SKUSEIVJ7YAGUPEACNI5UGI4DZCE",
            # master_mnemonic = "choice earth will mind edge captain hire suspect cross penalty lyrics obtain recall silly raise you differ similar wet relief above phone frame ability spoon"
